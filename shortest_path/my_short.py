...
import travel_file6 as travel6
import pandas as pd
...
#************************************* Start(Find SP-Trips ID)  *************************************
    ...
    #*********************************************************************************************
    error, similarity, coverability, no_path_trips = travel6.func(marginaldf,
    weightdf,filtered_move,vehicle_path,plate)
    errordf = pd.DataFrame({'plate':local_path2[k],'time error':error,'similarity':similarity})
    errordf_list.append(errordf)
    coverability_list.append(coverability)
errordf_list_all = pd.concat(errordf_list, axis=0, ignore_index = True)
errordf_list_all.to_csv(main_path + 'metric3' + '\\test_predictors1.csv')
coverability_df = pd.concat(coverability_list, axis=0, ignore_index = True)
coverability_df.to_csv(main_path + 'metric3' + '\\test_uncovered1.csv')
ass=1